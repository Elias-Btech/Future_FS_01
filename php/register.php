...
<title><?=t('auth_register')?> – Elias Araya</title>
...
.card{background:var(--card);border:1px solid var(--br);border-radius:var(--r);box-shadow:var(--sh);width:100%;max-width:440px;padding:40px 36px}
...
.pwb{flex:1;height:3px;border-radius:3px;background:var(--br);transition:background .3s}
.pwb.w{background:var(--err)}.pwb.m{background:#F59E0B}.pwb.s{background:var(--ok)}
...
<main class="main">
  <div class="card">
    ...
    <h1 class="card-title"><?=t('auth_register')?></h1>
    <p class="card-sub"><?=t('auth_register')?></p>
    ...
    <form class="form" id="rf" action="register_process.php" method="POST" novalidate>
      <input type="hidden" name="csrf" value="<?=$csrf?>"/>
      <div class="fg">
        <label for="nm"><?=t('auth_name')?></label>
        <div class="iw"><span class="ii">👤</span><input type="text" id="nm" name="name" placeholder="Elias Araya" autocomplete="name" required/></div>
        <div class="fm" id="nmE"></div>
      </div>
      <div class="fg">
      ...
      <div class="fg">
        ...
        <div class="iw"><span class="ii">🔒</span><input type="password" id="pw" name="password" placeholder="Min 8 characters" autocomplete="new-password" required/><button type="button" class="tpw" id="tpw">👁️</button></div>
        <div class="pws"><div class="pwb" id="b1"></div><div class="pwb" id="b2"></div><div class="pwb" id="b3"></div></div>
        <div class="fm" id="pwE"></div>
      </div>
      <div class="fg">
        <label for="cf"><?=t('auth_confirm_password')?></label>
        <div class="iw"><span class="ii">🔒</span><input type="password" id="cf" name="confirm" placeholder="Repeat password" autocomplete="new-password" required/></div>
        <div class="fm" id="cfE"></div>
      </div>
      <button type="submit" class="sbtn"><?=t('auth_register')?></button>
    </form>
    <p class="divider"><?=t('auth_already_have_account')?> <a href="login.php"><?=t('auth_login')?></a></p>
...
function str(p){let s=0;if(p.length>=8)s++;if(/[A-Z]/.test(p))s++;if(/[0-9]/.test(p))s++;if(/[^A-Za-z0-9]/.test(p))s++;return s}
pw.addEventListener('input',()=>{
  const s=str(pw.value),lv=s<=1?'w':s<=2?'m':'s';
  ['b1','b2','b3'].forEach(id=>document.getElementById(id).className='pwb');
  if(s>=1)document.getElementById('b1').classList.add(lv);
  if(s>=3)document.getElementById('b2').classList.add(lv);
  if(s>=4)document.getElementById('b3').classList.add(lv);
});
const cf2=document.getElementById('cf');
cf2.addEventListener('input',()=>{
  const e=document.getElementById('cfE');
  if(cf2.value&&cf2.value!==pw.value){cf2.classList.add('e');e.textContent='Passwords do not match.'}
  else{cf2.classList.remove('e');e.textContent=''}
});
const ER=/^[^\s@]+@[^\s@]+\.[^\s@]+$/;
document.getElementById('rf').addEventListener('submit',e=>{
  let ok=true;
  const nm=document.getElementById('nm'),nmE=document.getElementById('nmE');
  ...
  const cf3=document.getElementById('cf'),cfE=document.getElementById('cfE');
  [nm,em,pw2,cf3].forEach(x=>x.classList.remove('e'));
  [nmE,emE,pwE,cfE].forEach(x=>x.textContent='');
  if(!nm.value.trim()||nm.value.trim().length<2){nm.classList.add('e');nmE.textContent='Name must be at least 2 characters.';ok=false}
  if(!em.value.trim()||!ER.test(em.value.trim())){em.classList.add('e');emE.textContent='Enter a valid email.';ok=false}
  if(pw2.value.length<8){pw2.classList.add('e');pwE.textContent='Password must be at least 8 characters.';ok=false}
  else if(!/[A-Z]/.test(pw2.value)){pw2.classList.add('e');pwE.textContent='Must include an uppercase letter.';ok=false}
  else if(!/[0-9]/.test(pw2.value)){pw2.classList.add('e');pwE.textContent='Must include a number.';ok=false}
  if(pw2.value!==cf3.value){cf3.classList.add('e');cfE.textContent='Passwords do not match.';ok=false}
